...
import { faWindows, faClone } from '@fortawesome/free-solid-svg-icons';
...
Click on the Instance ID that looks like `i-xxxxxxxxxxxxxxxxx`. Next, Copy the IP Address under **Public IPv4 Address** or click on <FontAwesomeIcon icon={faClone} /> icon under **Public IPv4 Address**.
...
Open Remote Desktop Connection by pressing <kbd><FontAwesomeIcon icon={faWindows} /></kbd> + R and type `mstsc` and press Enter. Then Type in your EC2 Instance Public IP Address and press Enter. Then type in your username as `ubuntu` and password as you set earlier. Then click on **OK**.
